...
public class RedisClusterTest {
  ...
  final RedisOptions options = new RedisOptions().setType(RedisClientType.CLUSTER)
      ...
      .setMaxWaitingHandlers(128 * 1024).addEndpoint(SocketAddress.inetSocketAddress(7000, "127.0.0.1"))
  ...
  public void autoFindNodesAcross24Instances(TestContext should) {
    ...
    CompositeFuture.all(futures).setHandler(all -> {
      ...
      for (int i = 0; i < len; i++) {
        final String id = Integer.toString(i);
        clients.get(rnd.nextInt(clients.size())).send(cmd(SET).arg(id).arg(id), set -> {
          should.assertTrue(set.succeeded());
          clients.get(rnd.nextInt(clients.size())).send(cmd(GET).arg(id), get -> {
            should.assertTrue(get.succeeded());
            should.assertEquals(id, get.result().toString());
