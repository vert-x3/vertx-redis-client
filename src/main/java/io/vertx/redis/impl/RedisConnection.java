...
class RedisConnection {
  ...
  private final Vertx vertx;private final Context context;
  ...
  public RedisConnection(Vertx vertx, RedisOptions config, RedisSubscriptions subscriptions) {
    ...
    this.vertx = vertx;this.context = ctx;
    this.config = config;
  ...
  private boolean useSentinel() {
    ...
    reconnect = true;if (config.getSentinels().size() > 0 && config.getMasterName() != null) {
            return true;
        } else {
            return false;
        }
...
// create a netClient for the connection
        final NetClient client = vertx.createNetClient(config);        client.connect(port, host, asyncResult -> {
  ...
  void disconnect(Handler<AsyncResult<Void>> closeHandler) {
    ...
    reconnect = false;switch (state.get()) {
      case CONNECTING:
