...
You can share a Redis client between multiple verticles or instances of the same verticle.
Such a client should be created outside a verticle otherwise it will be closed when the verticle that created it is undeployed.

[source,$lang]
----
{@link examples.RedisExamples#example17}
----

You can also create a shared client in each verticle:

[source,$lang]
----
{@link examples.RedisExamples#example18}
----

The first time a shared client is created it will create the resources for the client (connection manager, pools, topology caches).
Subsequent calls will reuse this client and create a lease to it.
The resources are disposed after all leases have been closed.

When no specific pool name is set, the default shared name `{PoolOptions#DEFAULT_SHARED_NAME}` is used.
Multiple shared clients are distinguished by the pool name.

WARNING: The first caller wins on configuration. Mismatched options with the same shared name are not validated.

NOTE: Sharing is at the client level (includes `NetClient`, connection pools, and topology caches), not just a single connection.
Pub/sub and transactional commands still require a dedicated `connect()` (unchanged).
...
To connect to a Valkey server, use the `valkey://` or `valkeys://` connection string scheme.
For a type-safe API that only includes commands supported by Valkey, use the {@link io.vertx.redis.client.ValkeyAPI} interface instead of {@link io.vertx.redis.client.RedisAPI}.

CAUTION: Valkey support is only provided because Valkey implements the Redis protocol (RESP).
If Valkey starts diverging on the protocol level, this support will be dropped without replacement.
